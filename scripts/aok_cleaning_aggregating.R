...
#THIS SHOULD BE REMOVED WHEN DUKU PUTS 'info_state' back in the tool!!!
SSD_Filter <- read_csv("inputs/2020_05/SSD_Filter.csv")
aok_clean<-merge(x = aok_clean, y = SSD_Filter, by.x = "D.info_county", by.y = "county_join")
aok_clean<-rename(aok_clean, info_state = state_join)

#THIS BUTTER FUNCTION WORKS EVEN THOUGH THEY DOWNLOAD DATA WITH GROUPS
sm_cols_df<-butteR::extract_sm_option_columns(df = aok_clean,name_vector=colnames(aok_clean))
...
#JUST HAVE THIS HERE WHILE THE SPELLING MISTAKE FOR THIS INDICATOR IS HERE, HOPEFULLY WON'T BE NEEDED IN JUNE DATA
...
data_hex_pt_w_composite<-data_hex_pt %>%
  mutate(
#    idp_sites= ifelse(J.j2.idp_location=="informal_sites",1,0),   INDICATOR SEEMS TO HAVE BEEM REMOVED FROM TOOL
...
#bit messy but will put most in aok_set_paths and the rest to go at bottom of part 4
